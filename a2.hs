...
justify_aux column w (x:xs) = if column == w then '\n':justify_aux 0 w (x:xs) else x: justify_aux (column + 1) w xs
...
This is as we need to have 2 lists with the same amount of elements. Whereas in our case we have lists with varying elements in each list. It is also right-lazy.
Essentially with zipWith you need to the lists to have the same lengths.
