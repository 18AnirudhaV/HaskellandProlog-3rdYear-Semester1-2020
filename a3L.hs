...
getValuations []       = [[]]
getValuations (c:cs) =  (map ((c,True):) remaining_table) ++ (map ((c,False):) remaining_table) where 
  remaining_table = getValuations cs
...
evalF valu (ATOM c)            = findVal(find ((c ==) . fst) valu)
...
-- This function is used to make it easier to find the ATOM and returns the valuation of a particular variable as type maybe and then converted in to a boolean
findVal  :: Maybe (Variable, Bool) -> Bool
findVal (Just (var, val)) =  val
...
prove ctx phi = if prove_right (decompose [] ctx) phi ||elem phi (decompose [] ctx) || elem BOT (decompose [] ctx) 
  then True
  else False

  --do something like a map, apply a function through the elements of the list
...
decompose ctx1 (middle : ctx2) =
  case middle of
    ...
    IMPLIES phi psi -> if (prove (ctx1 ++ ctx2) phi)
...
prove_right ctx (AND phi1 phi2) = if ((prove ctx phi1) && (prove ctx phi2))
  then True
  else False
  
prove_right ctx (OR phi1 phi2) = if ((prove ctx phi1) || (prove ctx phi2))
  then True
  else False
